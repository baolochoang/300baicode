...
class Solution:
    def minimumOperationsToMakeKPeriodic(self, word: str, k: int) -> int:
        ...
        for i in range(0, n, k):
            sub_str = word[i: i+k]
            if sub_str not in freq_k_periodic:
                freq_k_periodic[sub_str] = 1
                continue
            freq_k_periodic[sub_str] += 1

        periodic_str = max(freq_k_periodic, key= lambda x: freq_k_periodic[x])
        min_operation = num_chunks - freq_k_periodic.get(periodic_str)
        
        return int(min_operation)